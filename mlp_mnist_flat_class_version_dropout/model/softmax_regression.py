...
class SoftmaxRegression:
    ...
    def construct_graph(self):
        ...
        self.x1 = self.fully_connected(self.x, self.w1, self.b1, activation=True, dropout=True)
        ...
        self.x2 = self.fully_connected(self.x1, self.w2, self.b2, activation=True, dropout=True)
        ...
        self.logits = tf.identity(self.fully_connected(self.x2, self.w3, self.b3, activation=True, dropout=True), name='logits')
    ...
    def fully_connected(self, x, w, b, batch_norm=False, activation=True, dropout=True):
        y = tf.matmul(x, w) + b
        if activation:
            y = self.activation(y)
        if batch_norm:
            y = tf.contrib.layers.batch_norm(y, center=True, scale=True, is_training=self.is_train)
        if dropout:
